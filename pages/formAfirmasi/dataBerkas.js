...
const DataBerkas = () => {
  ...
  const handleChangeFile3 = (e) => {
    ...
    const imgCount = sket.length
    if (imgCount + newImages.length > 5)
      return dispatch({ type: 'NOTIFY', payload: { error: 'Pilih 5 Gambar' } })
    setSket([...sket, ...newImages])
  }

  const handleSket = async (e) => {
    e.preventDefault()
    if (sket.length === 0)
      return dispatch({ type: 'NOTIFY', payload: { error: 'Tolong Isi Data' } })
    ...
    dispatch({ type: 'NOTIFY', payload: { loading: true } })
    let media = []
    const imgNewURL = sket.filter(img => !img.url)
    const imgOldURL = sket.filter(img => img.url)

    if (imgNewURL.length > 0) media = await imageUpload(imgNewURL)

    let res;
    res = await postData('data/sket', { ...sket, sket: [...imgOldURL, ...media] }, auth.token)
    if (res.err) return dispatch({ type: 'NOTIFY', payload: { error: res.err } })
          ...
          <div className="card">
            <div className="card-body">
              <form onSubmit={handleSubmit}>
                ...
                <div className="row align-items-center mb-3">
                  <div className="col col-md-3 ">
                    <label htmlFor="berkas1" className="form-label">Scan Surat Keterangan Yatim/Piatu<sup className="text-danger">*</sup></label>
                  </div>
                  <div className="col col-md-7">
                    <input type="file" id="berkas1" onChange={handleChangeFile3} multiple accept="image/*" className="form-control" />
                  </div>
                  <div className="col col-md-2">
                    <button type='button' className='btn btn-success' onClick={handleSket}>Upload</button>
                  </div>
                </div>
